...
public class IndexUtil {
	...
	private String[] contents = {
			...
			"i like movie", 
			"i love sports", 
			"i like movie and sports", 
			"i love swim like too"};
	private String[] names = {"zhangsan","lisi","john","jetty","tomcat","resin"};
	...
	public void searchByTermRange(String fieldName, String start, String end, int count) {
		try {
			IndexSearcher searcher = new IndexSearcher(indexReader);
	...
	public void searchByTermRangeNumeric(String fieldName, int min, int max, int count) {
		try {
			IndexSearcher searcher = new IndexSearcher(indexReader);
	...
	public void searchByBoolean(int count) {
		try {
			...
			booleanQuery.add(new TermQuery(new Term("content", "like")), Occur.MUST);
			booleanQuery.add(new TermQuery(new Term("name", "zhangsan")), Occur.MUST_NOT);
	...
	public void searchByFuzzy(String feildName, String valueLike, int count) {
		try {
			...
			FuzzyQuery query = new FuzzyQuery(new Term(feildName, valueLike), 0.2f);
			executeQuery(count, searcher, query);
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public void searchByPage(String fieldName, String searchValue, 
		...
		try {
			...
			QueryParser parser = new QueryParser(
					Version.LUCENE_35, fieldName, new StandardAnalyzer(Version.LUCENE_35));
			...
			if(preDocNum == -1) { // 第1页
				...
				topDocs = searcher.searchAfter(scoreDocs[preDocNum], query, pageSize);
			}
	...
	private void executeQuery(int count, IndexSearcher searcher,
